...
variable "dependencies" {
  type        = list(string)
  description = "List of Python packages to install in the Lambda layer."

  validation {
    condition     = alltrue([for dep in var.dependencies : can(regex("^[a-zA-Z0-9-_\\[\\]]+==\\d+(?:\\.\\d+)*$", dep))])
    error_message = "All `dependencies` must have exact version specified. Example: 'requests==2.28.1'"
  }
}
