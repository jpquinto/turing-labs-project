...
variable "package_json_path" {
  type        = string
  description = "Path to the package.json file containing Node.js dependencies."

  validation {
    condition     = can(regex("package\\.json$", var.package_json_path))
    error_message = "`package_json_path` must point to a package.json file."
  }
}

variable "package_lock_path" {
  type        = string
  description = "Path to the package-lock.json file. If not provided, will look in the same directory as package.json."
  default     = null
}
